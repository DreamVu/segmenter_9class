...
def main(model_path, input_dir, output_dir, gpu):
    ...
    for folder_name in os.listdir(input_dir):
        print("Inside folder :" + folder_name )
        os.makedirs(output_dir + "/overlaid/" + folder_name + "/", exist_ok=True)
        os.makedirs(output_dir + "/labels_gray/" + folder_name + "/", exist_ok=True)
        fnames = os.listdir(input_dir + folder_name + "/")
        for filename in fnames:
            pil_im = Image.open(input_dir + folder_name + "/" + filename).copy()
            ...
            seg_gray = seg_map.cpu().numpy().astype(np.uint8)
            pil_blend = Image.blend(pil_im, pil_seg, 0.5).convert("RGB")
            pil_blend.save(output_dir + "/overlaid/" + folder_name + "/" + filename)
            cv2.imwrite(output_dir + "/labels_gray/" + folder_name + "/" + filename, seg_gray[0])
    ...
    list_folder_dir = list(input_dir.iterdir())
    for folder_name in tqdm(list_folder_dir, ncols=80):
        output_list_dir = output_dir / Path(folder_name.name) 
        output_list_dir.mkdir(exist_ok=True)
        list_dir = list(folder_name.iterdir())
        ...
        for filename in tqdm(list_dir):
            ...
            pil_blend = Image.blend(pil_im, pil_seg, 0.5).convert("RGB")
            pil_blend.save(output_list_dir /  filename.name)'''
